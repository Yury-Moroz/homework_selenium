...
visitBizonСategories()
    ...
    async function  visitBizonPhoneDoubleClick(){
    const service = new Chrome.ServiceBuilder(path);
    const driver: WebDriver = await new Builder().forBrowser("chrome").setChromeService(service).build();
    await driver.manage().window().maximize();
    await driver.get("https://www.bizon.by/");
    await driver.actions().doubleClick(driver.findElement(By.css("#ft_menu > button > span > span.col"))).perform();
    await driver.actions().doubleClick(driver.findElement(By.css("#ft_menu > div > span:nth-child(3) > a"))).perform();
    await driver.actions().doubleClick(driver.findElement(By.xpath("//*[@id=\"content\"]/div[3]/div/div[1]/div/div[1]/a/img"))).perform();
    await driver.actions().click(driver.findElement(By.xpath("//*[@id=\"button-cart\"]/b"))).perform();
    await driver.sleep(2000)
    await driver.findElement(By.xpath("//*[@id=\"ft-popup-cart\"]/div/div/div[3]/div/div/a")).click();
    const phone: WebElement = await driver.findElement(By.xpath("//*[@id=\"input-payment-telephone\"]"));
        await phone.sendKeys("+375445558866");
        await driver.sleep(2000);
        await phone.clear();
        await driver.sleep(1000);
        await driver.actions().doubleClick(driver.findElement(By.xpath("/html/body/div[1]/div/ul/li[1]/a/i"))).perform();
        await driver.quit()
console.log(`correct data entry and double-click operation`)

};
visitBizonPhoneDoubleClick()
